<div class="space-y-6 px-4 pb-8 pt-4 sm:px-5 xl:px-6">
    ...
    <section class="overflow-hidden rounded-[1.15rem] border px-5 py-6 sm:px-6 xl:px-7" style="border-color: rgba(var(--theme-border-color-rgb), .68); background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), .12), transparent 36%), color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div>
            <div class="inline-flex items-center gap-2 rounded-md border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em]" style="border-color: rgba(var(--theme-border-color-rgb), .62); color: var(--theme-muted-text-color); background-color: color-mix(in srgb, var(--theme-surface-base) 80%, transparent);"><i class="fa-light fa-list-timeline"></i>{{ __('Automation') }}</div>
            <h1 class="mt-4 text-[2.2rem] font-semibold leading-tight tracking-[-0.055em]" style="color: var(--theme-header-text-color);">{{ __('Webhook Logs') }}</h1>
            <p class="mt-3 max-w-2xl text-sm leading-7" style="color: var(--theme-muted-text-color);">{{ __('Track queued, successful, failed, and skipped webhook automation calls.') }}</p>
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-border-color-rgb), .68); background-color: color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div class="grid gap-3 border-b px-5 py-4 xl:grid-cols-[minmax(0,1fr)_14rem_12rem_9rem]" style="border-color: rgba(var(--theme-border-color-rgb), .68);">
            <x-ui.input wire:model.live.debounce.300ms="search" name="search" :placeholder="__('Search URL, trigger, error...')" />
            ...
            <x-ui.select wire:model.live="status" name="status"><option value="all">{{ __('All status') }}</option><option value="queued">{{ __('Queued') }}</option><option value="success">{{ __('Success') }}</option><option value="failed">{{ __('Failed') }}</option><option value="skipped">{{ __('Skipped') }}</option></x-ui.select>
            ...
            <table class="min-w-full text-left text-sm">
                <thead style="color: var(--theme-muted-text-color);">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-[0.16em]">{{ __('Response') }}</th>
                ...
                <tbody class="divide-y" style="border-color: rgba(var(--theme-border-color-rgb), .58);">
                    @forelse ($logs as $log)
                        <tr>
                            ...
                            <td class="px-5 py-4"><x-ui.badge :variant="$log->status === 'success' ? 'success' : ($log->status === 'failed' ? 'danger' : ($log->status === 'skipped' ? 'warning' : 'neutral'))">{{ ucfirst($log->status) }}</x-ui.badge>@if($log->error_message)<p class="mt-1 max-w-xs truncate text-xs" style="color: var(--theme-danger-color);">{{ $log->error_message }}</p>@endif</td>
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <x-ui.dialog :title="$log->automation?->name ?: __('Webhook log')" :description="$log->webhook_url" width="xl" dismissible>
                                        <x-slot:trigger>
                                        ...
                                        <div class="space-y-4 text-sm" style="color: var(--theme-muted-text-color);">
                                            ...
                                            <div>
                                                <p class="mb-2 text-xs font-semibold uppercase tracking-[0.16em]">{{ __('Payload') }}</p>
                                                <pre class="max-h-80 overflow-auto rounded-xl border p-4 text-xs" style="border-color: rgba(var(--theme-border-color-rgb), .62); background-color: var(--theme-surface-base); color: var(--theme-header-text-color);">{{ json_encode($log->request_payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                                            </div>
                                            <div>
                                                <p class="mb-2 text-xs font-semibold uppercase tracking-[0.16em]">{{ __('Response') }}</p>
                                                <pre class="max-h-56 overflow-auto rounded-xl border p-4 text-xs" style="border-color: rgba(var(--theme-border-color-rgb), .62); background-color: var(--theme-surface-base); color: var(--theme-header-text-color);">{{ $log->response_body ?: __('No response body.') }}</pre>
                                            </div>
                                        </div>
                                    </x-ui.dialog>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-5 py-12 text-center text-sm" style="color: var(--theme-muted-text-color);">{{ __('No webhook logs yet.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
