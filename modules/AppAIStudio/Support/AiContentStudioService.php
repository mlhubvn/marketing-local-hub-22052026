...
class AiContentStudioService
{
        ...
        try {
            $payload = $this->requestJson(
                ...
                userPrompt: trim(implode("\n\n", [
                    'Create campaign-ready caption drafts for the provided brief.',
                    'Return JSON with keys: summary, variants.',
                    'variants must be an array of objects with keys: platform, caption, hook, hashtags, cta, notes. Use platform as the content format key.',
                    'Generate exactly '.$totalResults.' distinct caption variants in total.',
                    'Keep each caption ready to reuse in a Link Bio, QR campaign, or short social post.',
                    ...
                    'Content formats: '.implode(', ', $platforms).'.',
    ...
    {
        $targets = $this->normalizePlatforms($targets, ['support_reply', 'short_caption', 'campaign_summary', 'follow_up_message']);
        $language = $this->resolveLanguage($config, 'default_language');

        try {
            $payload = $this->requestJson(
                feature: 'ai-content-studio.repurpose',
                systemPrompt: 'You repurpose source content into reusable campaign formats. Return valid JSON only.',
                userPrompt: trim(implode("\n\n", [
                    'Rewrite the source content into reusable campaign snippets.',
                    'Return JSON with keys: strategy, items.',
                    'items must be an array of objects with keys: target, format, title, caption, notes.',
                    'Language: '.$language.'.',
                    'Tone: '.($config['tone'] ?? 'professional').'.',
                    ...$this->promptPreferenceLines($config),
                    'Target formats: '.implode(', ', $targets).'.',
        ...
        return [
            'strategy' => __('Repurposed locally into reusable campaign snippets with adjusted calls to action.'),
    ...
    public function generateLocalCampaignPlan(array $context): array
    {
        $requiredKeys = [
            'campaign_name',
            'headline',
        ...
        $response = $this->requestJson(
            ...
            userPrompt: trim(implode("\n", [
                ...
                'The plan must help the user create a real campaign in a SaaS product.',
                'Return JSON with exactly these keys:',
                implode(', ', $requiredKeys).'.',
                'campaign_name: max 65 characters.',
                'headline: max 90 characters.',
                'subheadline: max 180 characters.',
                'description: 1-2 concise sentences.',
                'cta: max 32 characters.',
                'benefits: array of exactly 3 short benefit strings.',
                'terms: short practical terms or empty string if not relevant.',
                'qr_poster_text: short text for a printed poster/card.',
                'social_caption: concise Facebook/Instagram caption.',
                'sms_message: max 240 characters.',
                'email_subject: max 80 characters.',
                'email_body: 3-5 short sentences.',
                'follow_up_message: short follow-up text.',
                'Write every JSON string in this language only: '.($context['language'] ?? 'English').'.',
                'If the language is English, use English only and keep campaign_name in normal English text. Do not mix scripts or languages.',
                '',
                'Business name: '.($context['business_name'] ?? ''),
                'Business type: '.($context['business_type'] ?? ''),
                'Goal: '.($context['goal'] ?? ''),
                'Campaign type: '.($context['campaign_type'] ?? ''),
                'Offer/promotion: '.($context['offer'] ?? ''),
                'Target customer: '.($context['target_customer'] ?? ''),
                'Tone: '.($context['tone'] ?? 'friendly'),
                'Language: '.($context['language'] ?? 'English'),
                'User brief: '.($context['prompt'] ?? ''),
            ])),
        );

        foreach ($requiredKeys as $key) {
            if (! array_key_exists($key, $response)) {
                throw new RuntimeException(__('AI returned incomplete campaign JSON.'));
            }
        }

        $response['benefits'] = collect((array) $response['benefits'])
        ...
        if (count($response['benefits']) < 3) {
            throw new RuntimeException(__('AI returned incomplete campaign benefits.'));
        }

        return collect($response)
            ->only($requiredKeys)
            ->map(fn ($value) => is_array($value) ? $value : trim((string) $value))
            ->all();
    }

    public function generateReviewReply(array $context): array
    {
        $requiredKeys = [
            'suggested_reply',
            'short_reply',
        ...
        $rating = max(1, min(5, (int) ($context['rating'] ?? 5)));
        $reviewText = trim((string) ($context['review_text'] ?? ''));

        if ($reviewText === '') {
            throw new RuntimeException(__('Add the customer review or feedback before generating a reply.'));
        }
        ...
        $response = $this->requestJson(
            ...
            userPrompt: trim(implode("\n", [
                ...
                'Return JSON with exactly these keys:',
                implode(', ', $requiredKeys).'.',
                'suggested_reply: the best complete reply, 2-4 concise sentences.',
                'short_reply: 1-2 concise sentences.',
                'professional_reply: polished and business-ready.',
                'friendly_reply: warmer and more conversational.',
                'If rating is 4 or 5, be grateful, specific, and invite the customer back.',
                'If rating is 1, 2, or 3, be empathetic, apologize for the experience, avoid admitting legal liability, and invite private follow-up.',
                'Never promise compensation unless the user explicitly provided it.',
                'Write every JSON string in this language only: '.($context['language'] ?? 'English').'.',
                'If the language is English, use English only. Do not mix scripts or languages.',
                '',
                'Business name: '.($context['business_name'] ?? ''),
                'Business type: '.($context['business_type'] ?? ''),
                'Rating: '.$rating.' out of 5',
                'Reply type: '.($context['reply_type'] ?? ($rating >= 4 ? 'Public review reply' : 'Private feedback recovery')),
                'Customer name: '.($context['customer_name'] ?? ''),
                'Tone: '.($context['tone'] ?? 'professional'),
                'Language: '.($context['language'] ?? 'English'),
                'Customer review or feedback: '.$reviewText,
            ])),
        );

        foreach ($requiredKeys as $key) {
            if (! array_key_exists($key, $response)) {
                throw new RuntimeException(__('AI returned incomplete review reply JSON.'));
            }
        }
    ...
    public function generateLocalMarketingContent(array $context): array
    {
        $requiredKeys = [
            'title',
            'generated_content',
        ...
        $response = $this->requestJson(
            ...
            userPrompt: trim(implode("\n", [
                ...
                'This is not a full campaign plan. Write one focused content piece for the selected content type.',
                'Return JSON with exactly these keys:',
                implode(', ', $requiredKeys).'.',
                'title: short internal title.',
                'generated_content: the main ready-to-use content.',
                'short_version: a shorter version.',
                'professional_version: a polished business-ready version.',
                'friendly_version: a warmer conversational version.',
                'cta_suggestions: array of 3 short CTA options.',
                'hashtags: array of relevant hashtags only when useful for social content, otherwise empty array.',
                'Keep copy specific to local business marketing and practical for customers.',
                'Write every JSON string in this language only: '.($context['language'] ?? 'English').'.',
                'If the language is English, use English only. Do not mix scripts or languages.',
                '',
                'Business name: '.($context['business_name'] ?? ''),
                ...
                'Goal or topic: '.($context['goal'] ?? ''),
                'Offer or promotion: '.($context['offer'] ?? ''),
                'Target customer: '.($context['target_customer'] ?? ''),
                'Tone: '.($context['tone'] ?? 'professional'),
                'Language: '.($context['language'] ?? 'English'),
                filled($context['brand_voice'] ?? null) ? 'Brand voice: '.trim((string) $context['brand_voice']) : null,
                filled($context['preferred_cta_style'] ?? null) ? 'Preferred CTA style: '.trim((string) $context['preferred_cta_style']) : null,
                filled($context['banned_words'] ?? null) ? 'Avoid these words or phrases: '.trim((string) $context['banned_words']) : null,
                'Extra details: '.($context['extra_details'] ?? ''),
            ])),
        );

        foreach ($requiredKeys as $key) {
            if (! array_key_exists($key, $response)) {
                throw new RuntimeException(__('AI returned incomplete content writer JSON.'));
            }
        }

        return [
            'title' => trim($this->stringValue($response['title'] ?? '')),
            'generated_content' => trim($this->stringValue($response['generated_content'] ?? '')),
            'short_version' => trim($this->stringValue($response['short_version'] ?? '')),
            'professional_version' => trim($this->stringValue($response['professional_version'] ?? '')),
            'friendly_version' => trim($this->stringValue($response['friendly_version'] ?? '')),
            'cta_suggestions' => collect((array) ($response['cta_suggestions'] ?? []))->map(fn ($value) => trim($this->stringValue($value)))->filter()->take(5)->values()->all(),
            'hashtags' => collect((array) ($response['hashtags'] ?? []))->map(fn ($value) => ltrim(trim($this->stringValue($value)), '#'))->filter()->take(10)->values()->all(),
        ];
    }
