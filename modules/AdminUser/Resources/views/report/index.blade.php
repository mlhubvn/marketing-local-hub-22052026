@component(theme_view('layouts.app', 'app'), ['title' => __('User Report')])
    @php
        $reportMetricCards = [
            ...
            ['label' => __('30-day growth'), 'value' => ($metrics['growth_rate'] >= 0 ? '+' : '').format_percent_locale($metrics['growth_rate']), 'description' => __('New users in the last 30 days: :count', ['count' => format_number_locale($metrics['new_users_last_30_days'])]), 'tone' => '#0ea5e9', 'progress' => min(100, max(8, abs((int) $metrics['growth_rate'])))],
            ['label' => __('Verified email'), 'value' => format_percent_locale($metrics['verification_rate']), 'description' => __(':count verified accounts', ['count' => format_number_locale($metrics['verified_users'])]), 'tone' => '#10b981', 'progress' => max(8, (int) $metrics['verification_rate'])],
            ['label' => __('Two-factor'), 'value' => format_percent_locale($metrics['two_factor_rate']), 'description' => __(':count secured accounts', ['count' => format_number_locale($metrics['two_factor_users'])]), 'tone' => '#f59e0b', 'progress' => max(8, (int) $metrics['two_factor_rate'])],
            ['label' => __('Users with plan'), 'value' => format_percent_locale($metrics['active_plan_rate']), 'description' => __(':count attached to an active or assigned plan', ['count' => format_number_locale($metrics['users_with_plan'])]), 'tone' => '#8b5cf6', 'progress' => max(8, (int) $metrics['active_plan_rate'])],
