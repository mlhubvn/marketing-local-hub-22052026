    ...
    <x-ui.datatable-shell :title="__('Team directory')" :info="__('Ownership, member counts, and team identity in a cleaner operational table.')" header-class="py-4" eyebrow-class="text-[10px] tracking-[0.2em]" title-class="mt-1 text-[1.15rem] tracking-[-0.025em]" description-class="mt-1 leading-6">
        <x-slot:toolbar>
            <div class="flex flex-wrap items-center gap-3">
                <x-ui.badge variant="primary">{{ format_number_locale($teamCount) }} {{ __('teams') }}</x-ui.badge>
                <x-ui.badge variant="neutral">{{ format_number_locale($ownerlessCount) }} {{ __('need owner') }}</x-ui.badge>
            ...
            <x-ui.table-body>
                @forelse ($teams as $team)
                    <x-ui.table-row>
                        ...
                        <x-ui.table-cell><x-ui.badge variant="success">{{ format_number_locale($team->members_count) }}</x-ui.badge></x-ui.table-cell>
