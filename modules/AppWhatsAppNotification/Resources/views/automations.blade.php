<div class="space-y-6 px-4 pb-8 pt-4 sm:px-5 xl:px-6" x-data="{ createOpen: false }" x-on:whatsapp-notification-saved.window="createOpen = false">
    @if ($statusMessage)
    ...
    <section class="overflow-hidden rounded-[1.15rem] border px-5 py-6 sm:px-6 xl:px-7" style="border-color: rgba(var(--theme-border-color-rgb), .68); background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), .12), transparent 36%), color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h1 class="mt-4 text-[2.2rem] font-semibold leading-tight tracking-[-0.055em]" style="color: var(--theme-header-text-color);">{{ __('WhatsApp Notifications') }}</h1>
                <p class="mt-3 max-w-2xl text-sm leading-7" style="color: var(--theme-muted-text-color);">{{ __('Send booking, coupon, lead, review, feedback, and customer follow-up WhatsApps from reusable rules.') }}</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-left text-sm">
                ...
                <tbody class="divide-y" style="border-color: rgba(var(--theme-border-color-rgb), .58);">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-12 text-center text-sm" style="color: var(--theme-muted-text-color);">{{ __('No WhatsApp Notifications yet.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            <div class="relative flex min-h-full items-start justify-center">
                <form wire:submit="save" x-show="createOpen" x-transition.opacity.scale.95 class="relative w-full max-w-4xl overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-border-color-rgb), .72); background-color: var(--theme-surface-overlay);">
                    <div class="flex items-start justify-between gap-4 border-b px-5 py-4" style="border-color: rgba(var(--theme-border-color-rgb), .68);">
                        <div><p class="text-lg font-semibold" style="color: var(--theme-header-text-color);">{{ __('Create WhatsApp Notification') }}</p><p class="mt-1 text-sm" style="color: var(--theme-muted-text-color);">{{ __('Trigger, timing, template, recipient.') }}</p></div>
                    ...
                    <div class="grid gap-4 p-5 md:grid-cols-2">
                        ...
                        <x-ui.select wire:model="whatsapp_template_id" name="whatsapp_template_id" :label="__('WhatsApp template')" :error="$errors->first('whatsapp_template_id')">
                        ...
                        <x-ui.select wire:model.live="delay_type" name="delay_type" :label="__('Timing')">
                            ...
                            <option value="before_booking">{{ __('Before booking time') }}</option>
                        </x-ui.select>
                        ...
                        <div class="md:col-span-2 rounded-[1rem] border p-4" style="border-color: rgba(var(--theme-border-color-rgb), .62);">
                            <p class="text-sm font-semibold" style="color: var(--theme-header-text-color);">{{ __('WhatsApp Cloud API') }}</p>
                            <p class="mt-1 text-xs" style="color: var(--theme-muted-text-color);">{{ __('Use your Meta Phone Number ID and access token. Add a template name when the message must use an approved Meta template.') }}</p>
                            <div class="mt-4 grid gap-3 md:grid-cols-2">
                                <x-ui.input wire:model="phone_number_id" name="phone_number_id" :label="__('Phone Number ID')" :error="$errors->first('phone_number_id')" />
                                <x-ui.input wire:model="access_token" name="access_token" type="password" :label="__('Access token')" :error="$errors->first('access_token')" />
                                <x-ui.input wire:model="template_name" name="template_name" :label="__('Approved template name')" :placeholder="__('Optional')" :error="$errors->first('template_name')" />
                                <x-ui.input wire:model="template_language" name="template_language" :label="__('Template language')" :placeholder="__('en_US')" :error="$errors->first('template_language')" />
                            </div>
                        </div>
                        <div class="md:col-span-2 rounded-[1rem] border p-4" style="border-color: rgba(var(--theme-border-color-rgb), .62);">
                            ...
                            <div class="mt-4 grid gap-3 md:grid-cols-3">
                                ...
                                @elseif ($condition_field === 'customer.phone')
