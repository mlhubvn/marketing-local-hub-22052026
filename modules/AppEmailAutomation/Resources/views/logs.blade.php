<div class="space-y-6 px-4 pb-8 pt-4 sm:px-5 xl:px-6">
    ...
    <section class="overflow-hidden rounded-[1.15rem] border px-5 py-6 sm:px-6 xl:px-7" style="border-color: rgba(var(--theme-border-color-rgb), .68); background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), .12), transparent 36%), color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div>
            ...
            <h1 class="mt-4 text-[2.2rem] font-semibold leading-tight tracking-[-0.055em]" style="color: var(--theme-header-text-color);">{{ __('Email Logs') }}</h1>
            <p class="mt-3 max-w-2xl text-sm leading-7" style="color: var(--theme-muted-text-color);">{{ __('Track queued, sent, and failed automation emails.') }}</p>
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-border-color-rgb), .68); background-color: color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div class="grid gap-3 border-b px-5 py-4 xl:grid-cols-[minmax(0,1fr)_14rem_12rem_12rem_9rem]" style="border-color: rgba(var(--theme-border-color-rgb), .68);">
            <x-ui.input wire:model.live.debounce.300ms="search" name="search" :placeholder="__('Search recipient, subject, trigger...')" />
            ...
            <table class="min-w-full text-left text-sm">
                <thead style="color: var(--theme-muted-text-color);">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-[0.16em]">{{ __('Subject') }}</th>
                ...
                <tbody class="divide-y" style="border-color: rgba(var(--theme-border-color-rgb), .58);">
                    @forelse ($logs as $log)
                        <tr>
                            ...
                            <td class="px-5 py-4"><p class="font-semibold" style="color: var(--theme-header-text-color);">{{ $log->recipient_name ?: __('No name') }}</p><p class="mt-1 text-xs" style="color: var(--theme-muted-text-color);">{{ $log->recipient_email }}</p></td>
                            <td class="px-5 py-4" style="color: var(--theme-muted-text-color);">{{ $log->automation?->name ?: $log->trigger_event }}</td>
                            <td class="px-5 py-4" style="color: var(--theme-muted-text-color);">{{ $log->subject }}</td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <x-ui.dialog :title="$log->subject" :description="$log->recipient_email" width="lg" dismissible>
                                        <x-slot:trigger>
                                        ...
                                        <div class="space-y-3 text-sm" style="color: var(--theme-muted-text-color);">
                                            ...
                                            <div class="rounded-xl border p-4" style="border-color: rgba(var(--theme-border-color-rgb), .62); background-color: var(--theme-surface-base);">{!! $log->body ?: e(__('No email body was generated.')) !!}</div>
                                        </div>
                                    </x-ui.dialog>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-5 py-12 text-center text-sm" style="color: var(--theme-muted-text-color);">{{ __('No email logs yet.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
