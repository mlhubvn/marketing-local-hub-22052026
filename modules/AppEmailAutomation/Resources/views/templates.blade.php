<div class="space-y-6 px-4 pb-8 pt-4 sm:px-5 xl:px-6" x-data="{ createOpen: false, previewOpen: false }" x-on:email-template-saved.window="createOpen = false" x-on:email-template-editing.window="createOpen = true" x-on:email-template-previewing.window="previewOpen = true">
    @if ($statusMessage)
    ...
    <section class="overflow-hidden rounded-[1.15rem] border px-5 py-6 sm:px-6 xl:px-7" style="border-color: rgba(var(--theme-border-color-rgb), .68); background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), .12), transparent 36%), color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-3 max-w-2xl text-sm leading-7" style="color: var(--theme-muted-text-color);">{{ __('Reusable email copy with LocalBoost variables for booking, coupon, lead, review, and feedback follow-up.') }}</p>
            </div>
    ...
    >
        <div class="flex flex-col gap-3 border-b px-4 py-4 sm:flex-row sm:items-center sm:justify-between" style="border-color: rgba(var(--theme-border-color-rgb), .58);">
            <div>
                ...
                <p class="mt-1 text-xs leading-5" style="color: var(--theme-muted-text-color);">{{ __('Click a variable to copy it, then paste it into the subject or body.') }}</p>
            </div>
            ...
            <article class="rounded-[1rem] border p-4" style="border-color: rgba(var(--theme-border-color-rgb), .62); background-color: color-mix(in srgb, var(--theme-surface-overlay) 98%, transparent);">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 truncate text-xs" style="color: var(--theme-muted-text-color);">{{ $template->subject }}</p>
                    </div>
            ...
            <div class="relative flex min-h-full items-start justify-center">
                <form wire:submit="save" x-show="createOpen" x-transition.opacity.scale.95 class="relative w-full max-w-4xl overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-border-color-rgb), .72); background-color: var(--theme-surface-overlay);">
                    <div class="flex items-start justify-between gap-4 border-b px-5 py-4" style="border-color: rgba(var(--theme-border-color-rgb), .68);">
                        <div><p class="text-lg font-semibold" style="color: var(--theme-header-text-color);">{{ $editingTemplateId ? __('Edit email template') : __('Create email template') }}</p><p class="mt-1 text-sm" style="color: var(--theme-muted-text-color);">{{ __('Use variables like {customer_name} and {business_name}.') }}</p></div>
                    ...
                    <div class="grid gap-4 p-5 md:grid-cols-2">
                        ...
                        <div class="md:col-span-2"><x-ui.input wire:model="subject" name="subject" :label="__('Subject')" :error="$errors->first('subject')" /></div>
                        <div class="md:col-span-2"><x-ui.input wire:model="preheader" name="preheader" :label="__('Preheader')" /></div>
        ...
        <div x-cloak x-show="previewOpen" class="fixed inset-0 z-[120] overflow-y-auto px-4 py-5 sm:px-6 sm:py-7" x-on:keydown.escape.window="previewOpen = false">
            ...
            <div class="relative flex min-h-full items-start justify-center">
                <section x-show="previewOpen" x-transition.opacity.scale.95 class="relative w-full max-w-3xl overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-border-color-rgb), .72); background-color: var(--theme-surface-overlay);">
                    <div class="flex items-start justify-between gap-4 border-b px-5 py-4" style="border-color: rgba(var(--theme-border-color-rgb), .68);">
                        <div><p class="text-lg font-semibold" style="color: var(--theme-header-text-color);">{{ $previewTemplate?->name ?: __('Template preview') }}</p><p class="mt-1 text-sm" style="color: var(--theme-muted-text-color);">{{ $previewTemplate?->subject }}</p></div>
                    ...
                    <div class="p-5">
                        <div class="rounded-[1rem] border p-5" style="border-color: rgba(var(--theme-border-color-rgb), .62); background-color: var(--theme-surface-base);">
                            <p class="text-xs font-semibold uppercase tracking-[0.16em]" style="color: var(--theme-muted-text-color);">{{ __('Preheader') }}</p>
                            <p class="mt-1 text-sm" style="color: var(--theme-muted-text-color);">{{ $previewTemplate?->preheader ?: __('No preheader') }}</p>
