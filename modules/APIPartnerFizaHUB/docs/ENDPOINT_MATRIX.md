...
Contract này là breaking cutover đã được Product duyệt ngày 2026-07-20. Public route set dưới
`/api/v1/partners/fizahub` có chính xác 22 request; không có compatibility alias.
...
| 18 | GET | `/api/v1/partners/fizahub/businesses/{external_business_id}/support-tickets/{ticket_id}` | `partner.fizahub.businesses.support-tickets.show` | `SupportTicketController::show` | Chi tiết ticket và hội thoại text, không có attachment. |
...
| 22 | POST | `/api/v1/partners/fizahub/businesses/{external_business_id}/crm-login-links` | `partner.fizahub.businesses.crm-login-links.store` | `OneTimeLoginController::store` | Tạo link CRM dùng một lần cho onboarding ready/completed. |
