@component(theme_view('layouts.app', 'app'), ['title' => __('Affiliate')])
    @php
        $metricCards = [
            ...
            ['label' => __('Available Balance'), 'value' => format_money((float) $profile->total_balance), 'description' => __('Current balance available for withdrawal.'), 'icon' => 'fa-light fa-wallet', 'tone' => 'rgb(217 119 6)', 'progress' => (float) $profile->total_balance > 0 ? 100 : 0],
            ['label' => __('Approved Total'), 'value' => format_money((float) $profile->total_approved), 'description' => __('Lifetime approved affiliate earnings.'), 'icon' => 'fa-light fa-sack-dollar', 'tone' => 'rgb(99 102 241)', 'progress' => (float) $profile->total_approved > 0 ? 100 : 0],
        ];
    @endphp
            ...
            <div class="grid gap-6 px-6 py-7 sm:px-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                ...
                <div class="flex flex-wrap items-center gap-3 lg:justify-end">
                    <span class="inline-flex items-center gap-2 rounded-full border px-3 py-2 text-sm font-medium" style="border-color: rgba(var(--theme-border-color-rgb), 0.62); background-color: rgba(var(--theme-surface-base-rgb,255,255,255),0.76); color: var(--theme-header-text-color); box-shadow: 0 12px 30px -24px rgba(15,23,42,0.28);">
                        ...
                        {{ __('Balance: :amount', ['amount' => format_money((float) $profile->total_balance)]) }}
                    </span>
                </div>
            </div>
        </section>
        ...
        <div class="grid gap-5 xl:grid-cols-2 xl:items-stretch">
            ...
            <x-ui.card class="flex h-full flex-col space-y-5">
                <div>
                    ...
                    <p class="text-sm" style="color: var(--theme-muted-text-color);">{{ __('Minimum withdrawal: :amount', ['amount' => format_money($minimumWithdrawal)]) }}</p>
                </div>
                    ...
                    <x-ui.table-body>
                        @forelse ($commissions as $commission)
                            <x-ui.table-row>
                                ...
                                <x-ui.table-cell><div class="space-y-1"><p class="text-sm font-semibold" style="color: var(--theme-header-text-color);">{{ format_money((float) $commission->commission) }}</p><p class="text-xs" style="color: var(--theme-muted-text-color);">{{ format_percent_locale((float) $commission->commission_rate) }}</p></div></x-ui.table-cell>
                    ...
                    <x-ui.table-body>
                        @forelse ($withdrawals as $withdrawal)
                            <x-ui.table-row>
                                ...
                                <x-ui.table-cell><span class="text-sm font-semibold" style="color: var(--theme-header-text-color);">{{ format_money((float) $withdrawal->amount) }}</span></x-ui.table-cell>
