<section class="w-full">
    @php
        ...
        $aiQrProfile = [
            'title' => __('AI QR Codes'),
            'note' => __('QR-controlled artwork only. Always uses Replicate AI QR settings below.'),
            'status' => 'ai_qr_status',
        ];
    @endphp
    ...
    <x-settings.layout :heading="__('AI Settings')" :subheading="__('Configure providers, credentials, and workload routing without forcing one model stack to do everything.')">
        <form wire:submit="save" class="my-6 w-full space-y-6">
            ...
            >
                                ...
                                @foreach ($card['fields'] as $field)
                                    <div class="space-y-2">
                                        ...
                                        @if (($field['type'] ?? 'text') === 'textarea')
                                            ...
                                            />
                                            <p class="text-xs leading-5" style="color: var(--theme-muted-text-color);">@lang('Supported placeholders:') @{{prompt}}, @{{negative_prompt}}, @{{qr_image}}, @{{width}}, @{{height}}.</p>
                                        @else
                                            <x-ui.input
                                                wire:model.defer="{{ $field['model'] }}"
                                                :type="$field['type']"
                ...
                <div class="rounded-[1.15rem] border p-5" style="border-color: rgba(var(--theme-primary-rgb), 0.24); background: linear-gradient(135deg, rgba(var(--theme-primary-rgb), 0.08), rgba(var(--theme-border-color-rgb), 0.03));">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div class="max-w-2xl">
                            <div class="flex flex-wrap items-center gap-2">
                                <h3 class="text-lg font-semibold" style="color: var(--theme-header-text-color);">{{ __('Replicate AI QR') }}</h3>
                                <span class="rounded-full px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.18em]" style="background-color: rgba(var(--theme-primary-rgb), 0.14); color: var(--theme-primary-color);">{{ __('Paid optional') }}</span>
                            </div>
                            <p class="mt-2 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ __('AI QR Codes uses the Replicate qr_code_controlnet model. Add only your Replicate API key here.') }}</p>
                        </div>

                        <div class="flex flex-wrap gap-2">
                            <x-ui.button href="https://replicate.com/account/api-tokens" target="_blank" type="button" variant="outline" size="sm">
                    ...
                    <div class="mt-5 max-w-xl">
                        <x-ui.input wire:model.defer="ai_replicate_api_key" type="password" :label="__('API key')" :error="$errors->first('ai_replicate_api_key')" autocomplete="new-password" />
                        <p class="mt-2 text-xs leading-5" style="color: var(--theme-muted-text-color);">
                            {{ __('Model, version, and input schema are fixed internally to keep AI QR generation scannable.') }}
                        </p>
                    </div>
                </div>
            </x-theme.section-card>
            ...
            >
                ...
                <div class="space-y-4 pt-2">
                    <div>
                        <p class="text-[0.72rem] font-semibold uppercase tracking-[0.28em]" style="color: var(--theme-muted-text-color);">{{ __('QR generation') }}</p>
                        <p class="mt-2 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ __('AI QR Codes has its own provider route because QR-controlled generation is different from normal image creation.') }}</p>
                    </div>

                    <div class="overflow-hidden rounded-[1.15rem] border" style="border-color: rgba(var(--theme-primary-rgb), 0.24);">
                        <div class="grid grid-cols-[1.1fr_0.72fr_0.95fr_1.15fr] gap-4 border-b px-5 py-3.5 text-[0.72rem] font-semibold uppercase tracking-[0.24em]" style="border-color: rgba(var(--theme-border-color-rgb), 0.68); color: var(--theme-muted-text-color); background-color: rgba(var(--theme-primary-rgb), 0.06);">
                            <span>{{ __('Workload') }}</span>
                            <span>{{ __('Status') }}</span>
                            <span>{{ __('Provider') }}</span>
                            <span>{{ __('Configured model') }}</span>
                        </div>

                        <div class="grid grid-cols-[1.1fr_0.72fr_0.95fr_1.15fr] gap-4 px-5 py-4">
                            <div>
                                <p class="text-sm font-semibold" style="color: var(--theme-header-text-color);">{{ $aiQrProfile['title'] }}</p>
                                <p class="mt-1 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ $aiQrProfile['note'] }}</p>
                            </div>

                            <x-ui.select wire:model.live="{{ $aiQrProfile['status'] }}" :label="false">
                                @foreach ($toggleOptions as $option)
                                    <option value="{{ $option['value'] }}">{{ __($option['label']) }}</option>
                                @endforeach
                            </x-ui.select>

                            <div class="rounded-[0.9rem] border px-3 py-2 text-sm font-semibold" style="border-color: rgba(var(--theme-border-color-rgb), 0.62); color: var(--theme-header-text-color); background-color: rgba(var(--theme-border-color-rgb), 0.03);">
                                {{ __('Replicate') }}
                            </div>

                            <div class="rounded-[0.9rem] border px-3 py-2 text-sm" style="border-color: rgba(var(--theme-border-color-rgb), 0.62); color: var(--theme-muted-text-color); background-color: rgba(var(--theme-border-color-rgb), 0.03);">
                                {{ $ai_replicate_model_slug ?: __('Not configured') }}
                            </div>
                        </div>
                    </div>
                </div>
