            ...
            <x-ui.table-body>
                @forelse ($logs as $log)
                    <x-ui.table-row>
                        ...
                        <x-ui.table-cell><div class="space-y-1"><p class="font-medium" style="color: var(--theme-header-text-color);">{{ __('Tokens: :count', ['count' => format_number_locale((int) ($log->total_tokens ?? 0))]) }}</p><p class="text-xs" style="color: var(--theme-muted-text-color);">{{ collect([$log->prompt_tokens !== null ? 'prompt: '.format_number_locale((int) $log->prompt_tokens) : null, $log->completion_tokens !== null ? 'completion: '.format_number_locale((int) $log->completion_tokens) : null, $log->estimated_cost !== null ? 'cost: '.format_number_locale((float) $log->estimated_cost, 6).' USD' : null, $log->latency_ms !== null ? 'latency: '.format_number_locale((int) $log->latency_ms).' ms' : null])->filter()->implode(' | ') ?: __('No usage metadata') }}</p>@if ($log->error_message)<p class="text-xs" style="color: var(--theme-danger-color);">{{ $log->error_message }}</p>@endif</div></x-ui.table-cell>
