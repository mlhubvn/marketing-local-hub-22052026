...
class AiConfig extends Component
{
    ...
    public string $ai_replicate_api_key = '';

    public string $ai_replicate_model_slug = 'zylim0702/qr_code_controlnet';

    public string $ai_replicate_version_override = '';

    public string $ai_replicate_input_template = '';

    public string $ai_replicate_template_preset = 'zylim_qr_code_controlnet';
    ...
    public function mount(): void
    {
        ...
        $legacyVersion = trim((string) $this->options->get('ai_replicate_model_version', ''));

        if ($this->ai_replicate_version_override === '' && $legacyVersion !== '') {
            $this->ai_replicate_version_override = $legacyVersion;
        }

        if (! in_array($this->ai_image_provider, $this->catalog->values('image_provider'), true)) {
            $this->ai_image_provider = 'gemini';
            $this->syncModelSelection('ai_image_model', 'image', $this->ai_image_provider);
        }

        $this->enforceAiQrReplicateDefaults();
    }
    ...
    public function save(): void
    {
        ...
        $validated = $this->validate([
            ...
            'ai_replicate_api_key' => ['nullable', 'string', 'max:500'],
            'ai_replicate_model_slug' => ['required', 'string', 'max:220', 'regex:/^[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+$/'],
            'ai_replicate_version_override' => ['nullable', 'string', 'max:220'],
            'ai_replicate_template_preset' => ['required', 'in:zylim_qr_code_controlnet,generic_qr_image,qr2ai_mb,qrcode_monster'],
            'ai_replicate_input_template' => ['nullable', 'string', 'max:5000', function (string $attribute, mixed $value, \Closure $fail): void {
                $value = trim((string) $value);

                if ($value !== '' && ! is_array(json_decode($value, true))) {
                    $fail(__('Enter a valid JSON object for the Replicate input template.'));
                }
            }],
    ...
    protected function stateKeys(): array
    {
        return [
            ...
            'ai_replicate_api_key' => '',
            'ai_replicate_model_slug' => 'zylim0702/qr_code_controlnet',
            'ai_replicate_version_override' => '',
            'ai_replicate_template_preset' => 'zylim_qr_code_controlnet',
            'ai_replicate_input_template' => $this->defaultReplicateInputTemplate(),
            'ai_content_status' => '1',
    ...
    protected function defaultReplicateInputTemplate(): string
    {
        return $this->replicateInputTemplateFor('zylim_qr_code_controlnet');
    }

    public function applyReplicateTemplatePreset(): void
    {
        $this->ai_replicate_input_template = $this->replicateInputTemplateFor($this->ai_replicate_template_preset);
    }

    public function resetReplicateTemplate(): void
    {
        $this->ai_replicate_template_preset = 'zylim_qr_code_controlnet';
        $this->ai_replicate_input_template = $this->defaultReplicateInputTemplate();
    }

    protected function enforceAiQrReplicateDefaults(): void
    {
        $this->ai_replicate_model_slug = 'zylim0702/qr_code_controlnet';
        $this->ai_replicate_version_override = '';
        $this->ai_replicate_template_preset = 'zylim_qr_code_controlnet';
        $this->ai_replicate_input_template = $this->defaultReplicateInputTemplate();
    }

    protected function replicateInputTemplateFor(string $preset): string
    {
        $template = match ($preset) {
            'zylim_qr_code_controlnet' => [
                'url' => '{{qr_value}}',
                'prompt' => '{{prompt}}',
    ...
    protected function replicateTemplatePresetOptions(): array
    {
        return [
            ['value' => 'zylim_qr_code_controlnet', 'label' => __('zylim0702/qr_code_controlnet')],
            ['value' => 'generic_qr_image', 'label' => __('Generic QR image')],
            ['value' => 'qr2ai_mb', 'label' => __('qr2ai/mb')],
            ['value' => 'qrcode_monster', 'label' => __('QR Code Monster fields')],
        ];
    }

    protected function replicateSetupChecklist(): array
    {
        return [
            __('Create a Replicate API token and paste it here.'),
            __('Enter the Replicate model slug, for example zylim0702/qr_code_controlnet.'),
            __('Leave version override blank to use the model latest version when available.'),
            __('Choose the preset that matches the model input names.'),
            __('Save settings. AI QR Codes always uses Replicate and does not affect normal image generation.'),
        ];
    }
    ...
    public function render(): View
    {
        return view('adminai::ai-config', [
            ...
            'replicateTemplatePresetOptions' => $this->replicateTemplatePresetOptions(),
            'replicateSetupChecklist' => $this->replicateSetupChecklist(),
