...
class FrontendThemePage extends Component
{
    ...
    protected function themeState(): array
    {
        ...
        $selectedFrontendTheme = $this->themes->find('guest', $frontendTheme)
            ?? $this->themes->find('guest', (string) config('themes.areas.guest.fallback', 'default'))
            ?? $this->themes->first('guest');

        if ($selectedFrontendTheme) {
            $frontendTheme = $selectedFrontendTheme->name;
        }
