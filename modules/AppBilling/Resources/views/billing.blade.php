@component(theme_view('layouts.app', 'app'), ['title' => __('Billing')])
    ...
    <div class="space-y-6">
        ...
        @foreach ([
            ...
            $user?->isInPlanTrial() ? ['icon' => 'fa-hourglass-clock', 'title' => __('Trial active'), 'text' => __('Your :plan trial is active until :date.', ['plan' => $plan?->name ?: __('current plan'), 'date' => $user?->trialEndsAt() ? format_datetime_locale($user->trialEndsAt()) : __('the trial end date')]), 'tone' => 'warning'] : null,
            $user?->nextPlan ? ['icon' => 'fa-calendar-clock', 'title' => __('Scheduled next plan'), 'text' => __('Your account is scheduled to switch to :plan (:cycle) when the current billing period ends on :date.', ['plan' => $user->nextPlan->name, 'cycle' => $nextBillingCycle, 'date' => $user->plan_expires_at ? format_date_locale($user->plan_expires_at) : __('the next billing date')]), 'tone' => 'accent'] : null,
        ] as $notice)
            @if ($notice)
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            @foreach ([
                ...
                ['label' => __('Next date'), 'value' => (($d = ($user?->isInPlanTrial() ? $user?->trialEndsAt() : $user?->plan_expires_at)) ? format_date_locale($d) : __('No expiry')), 'text' => $user?->isInPlanTrial() ? __('Trial ends') : __('Plan expiry'), 'icon' => 'fa-calendar-days', 'tone' => '#0ea5e9'],
            ] as $card)
