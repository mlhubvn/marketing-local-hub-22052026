                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <x-ui.dropdown-menu>
                                    ...
                                    <x-ui.dropdown-menu-item
                                        ...
                                        wire:click="updateMissing({{ $language->id }})"
                                        wire:loading.attr="disabled"
                                        wire:target="updateMissing({{ $language->id }})"
                                        wire:loading.class="opacity-70 cursor-wait"
                                        :close="false"
                                    >
                                        <span wire:loading.remove wire:target="updateMissing({{ $language->id }})">{{ __('Update languages') }}</span>
                                        <span wire:loading wire:target="updateMissing({{ $language->id }})">{{ __('Updating...') }}</span>
                                    </x-ui.dropdown-menu-item>
                                    <x-ui.dropdown-menu-item
                                        type="button"
                                        icon="fa-light fa-wand-magic-sparkles"
                                        wire:click="autoTranslate({{ $language->id }})"
                                        wire:loading.attr="disabled"
                                        wire:target="autoTranslate({{ $language->id }})"
                                        wire:loading.class="opacity-70 cursor-wait"
                                        :close="false"
                                    >
                                        <span wire:loading.remove wire:target="autoTranslate({{ $language->id }})">{{ __('Auto translate') }}</span>
                                        <span wire:loading wire:target="autoTranslate({{ $language->id }})">{{ __('Translating...') }}</span>
                                    </x-ui.dropdown-menu-item>
