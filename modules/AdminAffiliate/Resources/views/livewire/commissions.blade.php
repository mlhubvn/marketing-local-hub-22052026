@php
    $metricCards = [
        ...
        ['label' => __('Approved Value'), 'value' => format_money($summary['approved']), 'description' => __('Commission value already approved.'), 'icon' => 'fa-light fa-circle-check', 'tone' => 'rgb(5 150 105)', 'progress' => $summary['approved'] > 0 ? 100 : 0],
    ];
    $statusLabels = [
        'all' => __('All'),
        '0' => __('Pending'),
            ...
            <x-ui.table-body>
                @forelse ($commissions as $commission)
                    <x-ui.table-row>
                        ...
                        <x-ui.table-cell><div class="space-y-1"><p class="text-sm font-semibold" style="color: var(--theme-header-text-color);">{{ format_money((float) $commission->commission) }}</p><p class="text-xs" style="color: var(--theme-muted-text-color);">{{ format_money((float) $commission->amount) }} × {{ format_percent_locale((float) $commission->commission_rate) }}</p></div></x-ui.table-cell>
