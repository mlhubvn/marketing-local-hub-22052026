...
| `app/Support/helpers.php`                                                                                              | Helper toàn cục (auto-load qua `composer.json`). Định dạng VN: `format_number_locale`, `format_money`, `format_date_locale`, `format_datetime_locale`, `platform_format_settings()`; JS: `platform_format_config()` → `window.MLHUB_FORMAT`. Rà soát grep: `ARCHITECTURE_CHECKLIST.md` §5.1. |
