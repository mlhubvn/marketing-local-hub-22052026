...
| `modules/CustomMLHUB/`                                                                                               | Gói thị trường VN: `mlhub:install`, `mlhub:update`, `mlhub:sync-env-options` (env Coolify → `options` Admin Settings), super admin từ env, `config/env_options.php`, `mlhub_site_options.php`, `STARTING_ID=147123468`.                                                                                                                                                                |
...
- **Catalog chính thức MLHUB:** `PlanSeeder` upsert 13 gói theo slug: Free Đà Nẵng + Starter/Growth/Pro/Partner theo Monthly/Yearly/Lifetime. Chỉ `mlhub-free-da-nang` có `default_signup_plan=true`; user đăng ký mới được tự động gán gói này.
...
- **User legacy/manual chưa có gói:** `MLHUB_NO_PLAN_ACCESS_ENABLED=true` (mặc định) cung cấp fallback cho tài khoản có `plan_id=null`. Toàn bộ quyền + hạn mức đọc env `MLHUB_NO_PLAN_*` → `NoPlanAccess::permissionsFromEnv()` (ma trận + preset giai đoạn: `ARCHITECTURE_FEATURE.md` §1.1, danh sách env: `.env.example`). `User::planLimit()` / `canUsePlanFeature()` / dashboard **Current plan limits** dùng các số này. Sidebar hiển thị nhãn `MLHUB_NO_PLAN_ACCESS_LABEL` / badge **Free**.
