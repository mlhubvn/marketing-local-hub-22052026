...
- **Chưa chọn gói nhưng dùng ngay (free tier / showcase):** `MLHUB_NO_PLAN_ACCESS_ENABLED=true` (mặc định). Toàn bộ quyền + hạn mức đọc env `MLHUB_NO_PLAN_*` → `NoPlanAccess::permissionsFromEnv()` (ma trận + preset giai đoạn: `ARCHITECTURE_FEATURE.md` §1.1, danh sách env: `.env.example`). `User::planLimit()` / `canUsePlanFeature()` / dashboard **Current plan limits** dùng các số này. Sidebar hiển thị nhãn `MLHUB_NO_PLAN_ACCESS_LABEL` / badge **Free**.
