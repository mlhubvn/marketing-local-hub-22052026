                    ...
                    @forelse ($faqs as $faq)
                        <article class="lb-card lb-hover lb-reveal rounded-xl p-5" style="--lb-delay: {{ $loop->index * 60 }}ms;">
                            <button type="button" x-on:click="openFaq = openFaq === {{ $loop->index }} ? -1 : {{ $loop->index }}" class="flex w-full items-start justify-between gap-4 text-left">
                                ...
                                <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition" style="background: color-mix(in srgb, var(--lb-lime) 28%, #fff); color:#d64545;" x-bind:class="openFaq === {{ $loop->index }} ? 'rotate-45' : ''">
