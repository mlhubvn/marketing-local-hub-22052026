...
<x-ui.card>
    <div class="space-y-3 text-sm">
        <div class="flex items-center justify-between gap-4">
            ...
            <span class="font-medium" style="color: var(--theme-header-text-color);">
                {{ format_money((float) data_get($pricing, 'subtotal', 0), $plan->currency) }}
            </span>
        </div>
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <span class="font-medium" style="color: {{ (float) data_get($pricing, 'discount', 0) > 0 ? '#dc2626' : 'var(--theme-header-text-color)' }};">
                {{ (float) data_get($pricing, 'discount', 0) > 0 ? '-' : '' }}{{ format_money((float) data_get($pricing, 'discount', 0), $plan->currency) }}
            </span>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between gap-4">
        ...
        <span class="text-sm font-semibold" style="color: var(--theme-header-text-color);">
            {{ format_money((float) data_get($pricing, 'total', 0), $plan->currency) }}
        </span>
    </div>
</x-ui.card>
