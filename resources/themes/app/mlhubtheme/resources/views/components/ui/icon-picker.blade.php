...
@php
    ...
    $sizeMap = [
        'xs' => ['field' => 'h-8', 'browse' => 'h-8 w-8', 'preview' => 'h-6 w-6 rounded-[0.55rem]', 'input' => 'h-8 text-xs'],
        'sm' => ['field' => 'h-9', 'browse' => 'h-9 w-9', 'preview' => 'h-7 w-7 rounded-[0.6rem]', 'input' => 'h-9 text-[13px]'],
        'md' => ['field' => 'h-11', 'browse' => 'h-11 w-11', 'preview' => 'h-9 w-9 rounded-[0.75rem]', 'input' => 'h-11 text-sm'],
        'lg' => ['field' => 'h-12', 'browse' => 'h-12 w-12', 'preview' => 'h-10 w-10 rounded-[0.8rem]', 'input' => 'h-12 text-sm'],
        'xl' => ['field' => 'h-14', 'browse' => 'h-14 w-14', 'preview' => 'h-11 w-11 rounded-[0.9rem]', 'input' => 'h-14 text-base'],
        'xxl' => ['field' => 'h-16', 'browse' => 'h-16 w-16', 'preview' => 'h-12 w-12 rounded-[1rem]', 'input' => 'h-16 text-base'],
    ];
...
<x-ui.field :label="$label" :help="$help" :error="$error" {{ $attributes->only('class') }}>
    <div
        ...
        class="{{ $compact ? 'grid grid-cols-[minmax(0,1fr)_2.75rem] items-start gap-2' : 'space-y-3' }}"
    >
        <input
            x-ref="input"
            type="hidden"
        ...
        <div
            @class([
                'group rounded-[1rem] border px-3 transition focus-within:border-[var(--theme-accent)] focus-within:ring-4 focus-within:ring-[color:rgba(var(--theme-accent-rgb),0.10)]',
                $resolvedSize['field'] => $compact,
                'py-2' => ! $compact,
            ])
            style="border-color: var(--theme-border-color); background-color: var(--theme-input-surface);"
        >
            <div class="flex h-full items-center gap-3">
                <span
                    @class(['inline-flex shrink-0 items-center justify-center', $compact ? $resolvedSize['preview'] : 'h-10 w-10 rounded-[0.8rem]'])
                    style="background: rgba(var(--theme-accent-rgb),0.10); color: {{ $previewColor }};"
                >
                    <i x-bind:class="icon || '{{ $placeholder }}'" class="text-base"></i>
                </span>
                <input
                    type="text"
                    x-model="icon"
                    x-on:input="parseIcon(icon); syncInput()"
                    placeholder="{{ $placeholder }}"
                    @class(['min-w-0 flex-1 border-0 bg-transparent px-0 font-medium outline-none ring-0 focus:outline-none focus:ring-0', $compact ? $resolvedSize['input'] : 'h-10 text-sm'])
        ...
        >
            <x-slot:trigger>
                <button
                    type="button"
                    @class([
                        'inline-flex items-center rounded-[0.95rem] border text-sm font-semibold transition hover:-translate-y-px hover:shadow-sm',
                        $resolvedSize['browse'].' justify-center px-0' => $compact,
                        'h-10 w-full justify-between gap-3 px-3' => ! $compact,
                    ])
                    style="border-color: rgba(var(--theme-border-color-rgb),0.68); background: color-mix(in srgb, var(--theme-surface-base) 92%, transparent); color: var(--theme-header-text-color);"
                    title="{{ __('Browse Font Awesome icons') }}"
                >
                    <span @class(['inline-flex items-center gap-2', 'sr-only' => $compact])>
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-[0.65rem]" style="background: rgba(var(--theme-accent-rgb),0.10); color: var(--theme-accent);">
                            <i class="fa-light fa-icons"></i>
                        </span>
                        {{ __('Browse Font Awesome icons') }}
                    </span>
                    <i class="{{ $compact ? 'fa-light fa-icons text-sm' : 'fa-light fa-arrow-up-right text-xs' }}" style="color: {{ $compact ? 'var(--theme-accent)' : 'var(--theme-muted-text-color)' }};"></i>
                </button>
            </x-slot:trigger>
