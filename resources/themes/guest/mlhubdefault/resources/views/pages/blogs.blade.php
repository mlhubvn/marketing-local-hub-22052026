@component(theme_view('layouts.marketing', 'guest'), ['pageTitle' => $pageTitle])
    ...
    <div class="lb-page">
        <section class="lb-wrap lb-blog-hero pb-20 pt-16 lg:pt-20">
            <div class="lb-reveal grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,21rem)] lg:items-end">
                <div>
                    ...
                    <h1 class="lb-serif lb-hero-title mt-6">{{ __('Playbooks for local campaign growth') }}</h1>
                    <p class="lb-copy mt-4 max-w-2xl">{{ __('Read practical notes on review campaigns, booking pages, coupon claims, feedback flows, lead capture, QR pages, AI copy, and growth reports.') }}</p>
                </div>

                <form method="GET" action="{{ route('guest.blogs') }}" class="lb-card rounded-2xl p-4 lg:justify-self-end lg:w-full">
            ...
            @if ($featuredPost)
                <article class="lb-card lb-hover lb-reveal mt-8 overflow-hidden rounded-3xl">
                    <div class="grid lg:grid-cols-[1.08fr_0.92fr]">
                        <a href="{{ route('guest.blog-show', $featuredPost->slug) }}" class="lb-blog-visual block min-h-[20rem] p-6">
                            @if ($featuredPost->thumbnailUrl())
                                <img src="{{ $featuredPost->thumbnailUrl() }}" alt="{{ $featuredPost->titleForLocale() }}" class="h-full min-h-[20rem] w-full rounded-2xl object-cover" loading="lazy" onerror="this.remove();">
                            @else
            ...
            <div class="lb-card lb-blog-topic-panel lb-reveal mt-6 rounded-2xl p-5 sm:p-6" style="--lb-delay: 140ms;">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="min-w-0">
                        <p class="text-xs font-black uppercase tracking-[0.18em]" style="color: var(--lb-muted);">{{ __('Popular topics') }}</p>
                        <h3 class="lb-serif mt-1.5 text-2xl leading-none sm:text-3xl">{{ __('Learn the workflows that move local growth') }}</h3>
                    </div>
                    <span class="shrink-0 rounded-full px-3 py-1 text-xs font-black" style="background: color-mix(in srgb, var(--lb-lime) 30%, #fff); color:#506807;">{{ $blogs->total() }} {{ __('posts') }}</span>
                </div>
                <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6">
                    @foreach ([
                        ...
                        <div class="lb-blog-topic-chip flex items-start gap-2.5 rounded-xl border bg-white/80 p-3" style="border-color: var(--lb-line);">
                            <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" style="background: color-mix(in srgb, var(--lb-red) 9%, #fff); color: var(--lb-red);">
                                <i class="fa-light {{ $topic[0] }} text-sm"></i>
                            </span>
                            <div class="min-w-0">
                                <p class="text-xs font-black leading-snug">{{ $topic[1] }}</p>
                                <p class="mt-0.5 text-[10px] leading-4 line-clamp-2" style="color: var(--lb-muted);">{{ $topic[2] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
