...
@if ($supportsDark && $allowToggle)
    <div
        x-data="{
            mode: window.themeMode?.getMode?.() || 'light',
            resolved: window.themeMode?.getResolved?.() || 'light',
            sync(state = null) {
                this.mode = state?.mode || window.themeMode?.getMode?.() || 'light';
                this.resolved = state?.resolved || window.themeMode?.getResolved?.() || 'light';
            },
            setMode(mode) {
        ...
        style="border-color: rgba(var(--theme-border-color-rgb), 0.88); background: rgba(var(--theme-surface-bg-rgb), 0.82);"
    >
        <button
            type="button"
