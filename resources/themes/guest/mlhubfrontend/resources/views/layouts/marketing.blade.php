                ...
                <div>
                    <div class="flex min-h-[3.5rem] items-center justify-between gap-6">
                        ...
                        <div class="hidden items-center gap-2 lg:flex">
                                ...
                                <div x-data="{ openLanguage: false }" class="relative">
                                    ...
                                    <div x-cloak x-show="openLanguage" x-transition.origin.top.right class="absolute right-0 z-30 mt-3 w-56 overflow-hidden rounded-[1rem] border bg-white p-2 shadow-xl" style="border-color: rgba(var(--theme-border-color-rgb),0.85);">
                                        @foreach ($languages as $language)
                                            ...
                                            <a href="{{ route('language.switch', $language->code) }}" class="flex items-center gap-3 rounded-[0.8rem] px-3 py-2.5 text-sm font-semibold transition {{ $isActiveLanguage ? 'text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}" @if($isActiveLanguage) style="background:#ff5f5f;" @endif>
                            ...
                            @auth
                                <a href="{{ route('portal.dashboard') }}" class="inline-flex h-10 items-center justify-center rounded-full px-5 text-sm font-black text-white shadow-[0_14px_28px_-20px_rgba(255,95,95,0.62)]" style="background:#ff5f5f;">{{ __('Dashboard') }}</a>
                            @else
                                ...
                                <a href="{{ $signupEnabled ? route('register') : route('login') }}" class="inline-flex h-10 items-center justify-center rounded-full px-5 text-sm font-black text-white shadow-[0_14px_28px_-20px_rgba(255,95,95,0.62)]" style="background:#ff5f5f;">{{ __('Sign up') }}</a>
                            @endauth
                        </div>
                ...
                <div class="grid gap-10 lg:grid-cols-[1.1fr_0.8fr_0.8fr_0.8fr]">
                    ...
                    <div>
                        ...
                        <div class="mt-4 grid gap-3 text-sm font-bold text-neutral-600">
                            ...
                            @endforeach
                            @if ($contactEmail !== '')
                                <a href="mailto:{{ $contactEmail }}" class="hover:text-teal-700">{{ $contactEmail }}</a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <a href="{{ route('login') }}" class="mt-5 inline-flex h-11 items-center justify-center rounded-full px-5 text-sm font-bold text-white shadow-[0_14px_28px_-20px_rgba(255,95,95,0.62)]" style="background:#ff5f5f;">
                            {{ __('Start Free Trial') }}
                        </a>
                    </div>
                </div>
